...
export default function StageWizard({ stage, open, onClose, onCompleted, startBatchNumber = null }) {
  ...
  const calcTotalBatches = () => {
    ...
    return calcBlendBatchCount(stage.input_qty_lbs || 0, fullChopBatchLbs(product)) || 1;
  };
   ...
   const handleComplete = async () => {
     ...
     try {
      ...
      if (usesIngredientBatches && currentBatch) {
        ...
        // ── Food-safety temperature gates — checked FIRST, before ANY writes ──
        ...
        if (capKey === "chopping") {
          ...
          if (product?.cure_bucket_id && cureLbs > 0) {
            ...
            // Abort if cure inventory was short — nothing committed by the function and the
            // stage hasn't been marked completed yet, so the operator can just fix and retry.
        ...
        // Everything above succeeded — only NOW mark the stage completed. Marking it
        // earlier meant a failed deduction/FG push could leave a "completed" stage with
        // no inventory movement, and could let the auto-archive fire prematurely.
        await base44.entities.ProductionStage.update(stage.id, updates);
